...
func buildScrapersFromSettings(settings config.Settings) []Scraper {
	...
	for _, scraperCfg := range settings.TorrentScrapers {
		...
		case "nyaa":
			...
			if baseURL == "" {
				baseURL = "https://nyaa.si" // Default URL
			}
			category := scraperCfg.Config["category"]
			if category == "" {
				category = "1_2" // Default: Anime - English-translated
			}
			filter := scraperCfg.Config["filter"]
			if filter == "" {
	...
	if splitByService {
		if globalSettings.Filtering.Debrid != nil {
			applyUserFilterOverrides(&filterSettings, models.FilterSettings{
				...
				MaxResolution:          models.StringPtr(globalSettings.Filtering.Debrid.MaxResolution),

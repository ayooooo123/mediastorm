...
func main() {
	...
	// Cast receiver capability probing. Results are cached on disk and consulted
	// when a cast session is created; the probe itself only runs on demand.
	castCapabilitiesHandler := handlers.NewCastCapabilitiesHandler(
		settings.Cache.Directory,
		filepath.Join(settings.Transmux.HLSTempDirectory, "cast-probe"),
		settings.Transmux.FFmpegPath,
		settings.Server.Port,
	)
	videoHandler.SetCastCapabilities(castCapabilitiesHandler.Store())
	...
	// Receiver capability probing. The probe stream itself is unauthenticated
	// because the Cast receiver fetching it has no session token; it is a
	// four-second synthetic clip generated by this server.
	r.PathPrefix("/cast/probe/").HandlerFunc(castCapabilitiesHandler.ServeProbeAsset).Methods(http.MethodGet)
	r.HandleFunc("/admin/api/cast/receivers", adminUIHandler.RequireAuth(castCapabilitiesHandler.ListReceivers)).Methods(http.MethodGet)
	r.HandleFunc("/admin/api/cast/receivers/probe", adminUIHandler.RequireAuth(castCapabilitiesHandler.ProbeReceiver)).Methods(http.MethodPost)

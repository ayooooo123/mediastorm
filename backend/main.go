...
func main() {
	...
	// Cast receiver capability tracking. Identity is read passively over HTTP
	// and graded from playback the user requested; nothing is ever loaded on a
	// receiver to find out.
	castCapabilitiesHandler := handlers.NewCastCapabilitiesHandler(settings.Cache.Directory)
	...
	// Receiver capability reporting. Both routes are read-only: discovery is a
	// LAN port sweep and describe is a passive HTTP read of the device.
	r.HandleFunc("/admin/api/cast/receivers", adminUIHandler.RequireMasterAuth(castCapabilitiesHandler.ListReceivers)).Methods(http.MethodGet)
	r.HandleFunc("/admin/api/cast/receivers/describe", adminUIHandler.RequireMasterAuth(castCapabilitiesHandler.DescribeReceiver)).Methods(http.MethodGet)

...
var defaultEnabledLeagueIDs = []string{"pga", "boxing", "cricket-8048", "nfl", "college-football", "nba", "mens-college-basketball", "womens-college-basketball", "wnba", "mlb", "nhl", "soccer-fifa.world", "soccer-fifa.wwc", "soccer-eng.1", "soccer-eng.2", "soccer-esp.1", "soccer-ger.1", "soccer-ita.1", "soccer-fra.1", "soccer-usa.1", "soccer-usa.nwsl", "soccer-usa.nwsl.cup", "soccer-uefa.champions", "soccer-uefa.europa", "soccer-uefa.europa.conf", "soccer-mex.1", "soccer-ned.1", "soccer-por.1", "ufc", "atp", "wta", "f1", "nascar", "motogp", "indycar", "rugby-180659", "rugby-164205", "rugby-267979", "rugby-242041", "rugby-270559", "rugby-league-3", "aso:tour", "aso:vuelta", "aso:tour-femmes", "aso:paris-nice", "aso:vuelta-femenina", "aso:paris-roubaix", "aso:paris-roubaix-femmes", "aso:liege-bastogne-liege", "aso:liege-bastogne-liege-femmes", "aso:fleche-wallonne", "aso:fleche-wallonne-femmes", "rcs:giro", "cro:cro-race", "uci:road-worlds"}
...
func DefaultHomeShelfConfigs() []ShelfConfig {
	return []ShelfConfig{
		...
		{ID: "my-recently-aired", Name: "My Recently Aired", Enabled: true, Order: 7, CalendarSources: CalendarSourceSettings{Watchlist: boolPtr(true), History: boolPtr(false), Trending: boolPtr(false), TopTrending: boolPtr(false), MDBLists: boolPtr(false)}},
...
func EnsureDefaultHomeShelves(shelves []ShelfConfig) ([]ShelfConfig, bool) {
	...
	// Tonight is experimental. Disable any materialized copy during startup
	...
	// Rename only legacy generated TMDB names. User-provided shelf names remain
	// untouched, including custom names for the same source types.
	for i := range nextShelves {
		shelf := &nextShelves[i]
		...
		var nextName string
		switch strings.ToLower(strings.TrimSpace(shelf.TMDBSourceType)) {
		case "public-list":
			if shelf.Name == "TMDB List" {
				nextName = "List"
			}
		case "movie-collection":
			if shelf.Name == "TMDB Movie Collection" {
				nextName = "Movie Collection"
			}
		case "custom-discover":
			if shelf.Name == "TMDB Discover" {
				nextName = "Discover"
			}
		}
		if nextName != "" {
...
func (m *Manager) Load() (Settings, error) {
	...
	// Migrate servicePriority from streaming to filtering (moved to Ranking & Filtering section)
	if streamingRaw, ok := raw["streaming"].(map[string]interface{}); ok {
		if servicePriority, hasPriority := streamingRaw["servicePriority"]; hasPriority {
			// Move it to filtering section
			if filteringRaw, ok := raw["filtering"].(map[string]interface{}); ok {
				if _, alreadySet := filteringRaw["servicePriority"]; !alreadySet {
					filteringRaw["servicePriority"] = servicePriority
				}
			} else {
				raw["filtering"] = map[string]interface{}{"servicePriority": servicePriority}
			}
			// Remove from streaming
			delete(streamingRaw, "servicePriority")
		}
	}
	...
	if playbackRaw, ok := raw["playback"].(map[string]interface{}); ok {
		...
		if _, exists := playbackRaw["liveClosedCaptionExtraction"]; !exists {
			playbackRaw["liveClosedCaptionExtraction"] = true
		}

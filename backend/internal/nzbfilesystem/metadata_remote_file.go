...
func (mvf *MetadataVirtualFile) getRequestRange() (start, end int64) {
	...
	if !mvf.readerInitialized && mvf.originalRangeEnd == 0 {
		// First, check if Seek() was called before any reads (position != 0)
		// This happens when HLS seeking calculates a byte offset and seeks the pipe
		if mvf.position > 0 {
			// Seek was called, use the seeked position
			mvf.originalRangeEnd = -1 // Unbounded from seek position
			return mvf.position, -1
		}

		// Otherwise check for HTTP range header from args
		...
		if err == nil && rangeHeader != nil {
			...
			// No range header and no seek, start from beginning unbounded
			mvf.originalRangeEnd = -1
			return 0, -1
		}
	}

	// For subsequent reads, start from the current position and respect any original end range
	...
	if fileSize <= 0 {
		// Degenerate case: treat as empty file
		return 0, -1
	}
	...
	switch mvf.originalRangeEnd {
	case -1:
		// Unbounded original request keeps streaming until EOF
		return start, -1
	default:
		end = mvf.originalRangeEnd
		maxEnd := fileSize - 1
		if end < 0 || end > maxEnd {
			end = maxEnd
		}
...
func (mvf *MetadataVirtualFile) createUsenetReader(ctx context.Context, start, end int64) (io.ReadCloser, error) {
	...
	maxSegments := mvf.maxWorkers * 8 // 8 segments per worker for better streaming
	if maxSegments > 100 {
	...
	// PERFORMANCE OPTIMIZATION: For very large files, implement progressive loading
	// If the requested range is very large, limit it to prevent excessive memory usage
	requestedRangeSize := end - start + 1
	maxRangeSize := int64(maxSegments) * 1024 * 1024 // 1MB per segment estimate

	if requestedRangeSize > maxRangeSize {
		// Limit the end to prevent excessive memory usage
		newEnd := start + maxRangeSize - 1
		if newEnd > mvf.fileMeta.FileSize-1 {
			newEnd = mvf.fileMeta.FileSize - 1
		}
		end = newEnd
	}
	...
	if requestedRangeSize > 50*1024*1024 { // 50MB+ requests are likely streaming
		maxSegments = mvf.maxWorkers * 12 // 12 segments per worker for streaming
		if maxSegments > 150 {
